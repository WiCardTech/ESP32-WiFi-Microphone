/*
 * WiFi Microphone Arduino program For ESP32 (12bit@8KHz - Free Revision)
 * Rev 1.2
 * 
 * Tested with esp32 board 3.2.0 and DOIT ESP32 Devkit V1
 * 
...
void configPage_AC()
{
...
#endif
    Server.send(200, "text/html", "<html><head><meta http-equiv='refresh' content='100; url=/config'><title>Connecting...</title></head><body>Please Wait...</body></html>");
  }
  else if(Server.arg("b") != NULL && Server.arg("p") != NULL && Server.arg("a") != NULL)//rev 1.2
  {
    ...
    Server.send(200, "text/html", "<html><head><title>Configuration Saved</title></head><body>Saved.</body></html>");
  }
  else
  {
    ...
    if(ucWMBitrate == 0)
      content += " selected";
    content += ">72 Kbps</option>";
    
    content += "<option value='1'";
    if(ucWMBitrate == 1)
      content += " selected";
    content += ">84 Kbps</option>";
    
    content += "<option value='2'";
    if(ucWMBitrate == 2)
      content += " selected";
    content += ">96 Kbps</option>";
